...
Pre and post processing will include: Normalization and scaling of amplitude values, Conversion of signals from times to frequencies, Extraction of statistical features (RMS, F1 score, accuracy, ROC, AUC), Labeling each signal by fault type, Data inspection (check for outliers, null values, etc).
...
Baseline: K-Nearest Neighbors (KNN) and Support Vector Machine (SVM) using extracted time/frequency-domain features. These can help verify the effectiveness of the pre-processing and also help with nonlinear relations, which can then be applied to other models. 
